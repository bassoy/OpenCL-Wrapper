/**
 * GEMM reconstructed from "Performance Tuning of Matrix Multiplication in OpenCL on Different GPUs and CPUs"
 * by Matsumoto, Nakasato and Sedukhin, 2011
 * 
 * @see 
 * 
...
class Matsumoto2011Pass : public utl::ProfilePass< Type >
{
public :
  Matsumoto2011Pass( std::string const& kernelName, std::istream& source, utl::Dim const& start, utl::Dim const& step, utl::Dim const& end, std::size_t iter = 10 );
...
Matsumoto2011Pass::Matsumoto2011Pass( std::string const& kernelName, std::istream& source, utl::Dim const& start, utl::Dim const& step, utl::Dim const& end, std::size_t iter ):
  ProfilePass< Type >( "Matsumoto2011", start, step, end, iter ),
  testing_( false ),
  ...
  program_( (context_.setActiveQueue( queue_ ), context_), utl::type::Single )
{
  ...
  if ( program_.isBuilt() )
  {
    ...
    ocl::Kernel& kernel( program_.kernel( kernelName ) );
...
double Matsumoto2011Pass::prof( utl::Dim const& dim )
{
...
double Matsumoto2011Pass::ops( utl::Dim const& dim )
{
  // N * M * (L + (L - 1))
...
int main( int argc, char** argv )
{
  ...
  if ( args.size() == 2 )
  {
    ...
    if ( file.is_open() )
    {
      ...
      mgr << std::make_shared< Matsumoto2011Pass >( "gemm_ba", file, utl::Dim( 256, 256, 256 ), utl::Dim( 16, 16, 16 ), utl::Dim( 256, 256, 256 ) );
      mgr << std::make_shared< Matsumoto2011Pass >( "gemm_pl", file, utl::Dim( 256, 256, 256 ), utl::Dim( 16, 16, 16 ), utl::Dim( 256, 256, 256 ) );
      mgr << std::make_shared< Matsumoto2011Pass >( "gemm_db", file, utl::Dim( 256, 256, 256 ), utl::Dim( 16, 16, 16 ), utl::Dim( 256, 256, 256 ) );
